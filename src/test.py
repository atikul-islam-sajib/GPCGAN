...
class Test:
    ...
    def plot_synthetic_image(self):
        ...
        real_labels = ["Class-{}".format(index) for index in range(0, 10)]
        noise_samples = torch.randn(self.num_samples, self.latent_space)
        labels = torch.randint(0, len(real_labels), (self.num_samples,))
        synthetic_samples = self.generator(noise_samples, labels)
